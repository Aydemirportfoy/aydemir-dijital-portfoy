...
import PhotoOrganizer, {
  photoFileKey,
  type CoverRef,
} from "@/components/admin/PhotoOrganizer";
import {
  safeFileName,
  slugify,
} from "@/lib/format";
...
export default function ListingEditor({
  ...
  const [coverRef, setCoverRef] =
    useState<CoverRef>(() => {
      const matched =
        initialListing?.cover_image_url
          ? initialImages.find(
              (image) =>
                image.image_url ===
                initialListing.cover_image_url,
  ...
  useEffect(() => {
    ...
    try {
      ...
      setNotice({
        type: "info",
        text:
          "Hızlı giriş bilgileri forma " +
          "aktarıldı. Eksik alanları " +
          "kontrol edip fotoğrafları " +
          "ekleyin.",
      });
    } catch {
      sessionStorage.removeItem(
  ...
  async function submit(
    ...
    if (videoFile) {
      setUploadProgress(0);
      setUploadStage(
        "İlan klibi yükleme için hazırlanıyor",
      );
    }
        ...
        const positionResult =
          item.kind === "existing"
            ? await withTimeout(
                ...
                20000,
                "Kapak sırası kaydedilirken süre aşıldı.",
              )
            : await withTimeout(
                positionQuery.eq(
                  "storage_path",
        ...
        <div className="ap-section-heading-row">
          <div>
            ...
            <p className="ap-muted">
              ...
              kapak seçmek için bilgisayarda
              çift tıklayın veya telefonda
              basılı tutun.
            </p>
          </div>
